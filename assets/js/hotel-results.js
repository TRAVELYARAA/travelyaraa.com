...
const S = {
  ...
  guestIndex:0, detailHotel:null, detailStatus:'idle', detailError:'', detailRequestSeq:0, detailRequestHid:'',
  resultsScrollY:0,
...
function roomImagesOf(o, hotel){
  const out=[];
  const isBathroomish=function(meta){
    const s=lower(meta||'');
    return /bath|toilet|washroom|restroom|lavatory|wc\b|shower only|bathroom/.test(s) && !/bedroom|living|suite|room interior|guest room/.test(s);
  };
  const isBedroomish=function(meta){
    const s=lower(meta||'');
    return /bed|bedroom|guest room|room interior|suite|living|king|queen|twin|double bed/.test(s);
  };
  const push=function(u, meta){
    const s=mediaUrl(u)||String(u&&(u.url||u.src||u.imageUrl||u.image)||u||'').trim();
    if(!s||!/^https?:\/\//i.test(s)||out.indexOf(s)>=0) return;
    const tag=meta||(u&&typeof u==='object'?(u.category||u.type||u.caption||u.alt||u.name||u.roomType||''):'');
    out.push({url:s, meta:String(tag||''), bath:isBathroomish(tag), bed:isBedroomish(tag)});
  };
  const walk=function(node, depth, metaHint){
    if(depth>5||node==null) return;
    if(typeof node==='string'){ push(node, metaHint); return; }
    if(Array.isArray(node)){ node.forEach(function(x){ walk(x, depth+1, metaHint); }); return; }
    if(typeof node!=='object') return;
    const meta=node.category||node.type||node.caption||node.alt||node.name||node.roomType||metaHint||'';
    push(node.url||node.src||node.imageUrl||node.image||node.hi||node.tns||node.sz||node.lurl||node.burl, meta);
    ['images','imgs','photos','gallery','media','tp','typedPhotos','roomImages','imageList'].forEach(function(k){ if(node[k]) walk(node[k], depth+1, meta); });
  };
  const rooms=arr(o&&o.rooms).concat(arr(o&&o.roomInfo), arr(o&&o.ris));
  rooms.forEach(function(r){
    if(!r) return;
    const nm=r.name||r.roomName||r.roomType||r.rt||'';
    walk(r.images||r.imgs||r.photos||r.media||r.tp,0,nm);
    walk(r.raw&&(r.raw.images||r.raw.imgs||r.raw.photos||r.raw.tp),0,nm);
    push(r.image||r.imageUrl||r.heroImage, nm);
    push(r.raw&&(r.raw.image||r.raw.imageUrl), nm);
  });
  walk(o&&o.raw&&(o.raw.images||o.raw.imgs||o.raw.photos||o.raw.tp),0,o&&(o.roomSummary||o.roomType));
  push(o&&o.raw&&(o.raw.image||o.raw.heroImage), o&&(o.roomSummary||o.roomType));
  const finish=function(list){
    const beds=list.filter(function(x){ return x.bed && !x.bath; }).map(function(x){ return x.url; });
    if(beds.length) return beds;
    const nonBath=list.filter(function(x){ return !x.bath; }).map(function(x){ return x.url; });
    if(nonBath.length) return nonBath;
  ...
  if(out.length) return finish(out);
  {
    ...
    contentRooms.forEach(function(r){
      ...
      if(/exterior|facade|façade|front elevation|building front|reception|lobby|restaurant|pool exterior|bath|toilet|washroom/i.test(nm)) return;
      walk(r.images||r.imgs||r.photos||r.media||r.tp,0,nm);
      push(r.image||r.imageUrl||r.heroImage, nm);
    });
  }
...
function draftBelongsToHotel(h){
  const d=draft()||{};
  const draftHid=realHotelId(d.hotel||d.selected||{});
  const want=realHotelId(h);
  return !!(draftHid && want && draftHid===want);
}
function hotelTicketSellAmount(o,h){
  // Markup-inclusive customer ticket / Room Price basis (resultDisplayAmount). Never supplier-only totalPrice.
...
function hotelCustomerPayableAmount(o,h){
  // Guest/Payment TOTAL only (Room Price + Taxes & Fees). Never use for Results/Room card display.
...
function hotelBaseFareAmount(o,h){
  // Results + Room Selection: LIVE markup-inclusive ticket only.
  // Never read draft locks here — that painted Hotel A prices under Hotel B after Back.
  o=o||option(); h=h||hotel();
  const pb=hotelPricingBreakup(o,h);
  return hotelMoneyRound(Math.max(0, Number(
    priceOf(o,h)
    || firstPositiveAmount(pb.ticketAmount, pb.resultDisplayAmount, pb.displayPrice, o.resultDisplayAmount, o.displayPrice, o.displayTotal, h&&h.resultDisplayAmount, h&&h.displayPrice, h&&h.price)
    || 0
  )));
}
...
function hotelFareParts(d){
  ...
  if(serviceFee<=0 && !feeWaived){
    if(draftBelongsToHotel(h)){
      const draftFee=Number(d.travelYaraaServiceFee||d.serviceFee||d.convenienceFee||0);
      if(draftFee>0) serviceFee=draftFee;
    }
    if(serviceFee<=0){
      const rawFee=hotelRawFeeAmount(o,h);
      if(rawFee>0) serviceFee=hotelMoneyRound(rawFee);
    }
  }
...
function hotelRulesHtml(h,o,raw){
  ...
  const descObj=(content.descriptions&&typeof content.descriptions==='object')?content.descriptions:{};
  const hInfo=raw.hInfo||(rawOf(h).hInfo)||{};
  const optRaw=o.raw||{};
  const rows=[];
  function isLocationish(t){
    return /nearby|attraction|km\s*\/|mi\s|airport|within a \d+-minute drive|distance|landmark|located in|with a stay at/i.test(String(t||''));
  }
  ...
  // Never treat About/location/attraction copy as hotel rules.
  const notes=[].concat(
  ...
  notes.forEach(function(x){
    ...
    if(/tripjack|tj\s*cash|razorpay|payment gateway|supplier|backend|\bapi\b|provider/i.test(t)) return;
    if(isLocationish(t)) return;
    if(descObj.location && t===String(descObj.location).trim()) return;
    if(descObj.attractions && t===String(descObj.attractions).trim()) return;
    if(rows.some(function(r){ return r.text===t; })) return;
  ...
  // Short generic TravelYaraa guidance only — not property-specific invented rules.
  const general=[
    'Carry valid government-issued photo identification required for check-in.',
    'Guest names should match the identification presented at the property.',
    'Property-specific charges, deposits, or local taxes may apply where disclosed by the hotel.'
  ];
  return '<div class="tyh-rules-fallback"><p class="tyh-rules-source">General stay guidance</p><p class="tyh-muted">No hotel-specific rules were returned for this property. The guidance below is general TravelYaraa stay information.</p><ul>'+general.map(function(t){ return '<li>'+esc(t)+'</li>'; }).join('')+'</ul></div>';
}
...
function hasPricingReviewContext(h){ return S.detailStatus==='ready' && hotelDetailContextMatches(h) && !!realHotelId(h) && !!realReviewHash(h) && optionList(h).some(function(o){ return !!realOptionId(o); }); }
function roomOptionDetailsHtml(o, hotel){
  o=o||{}; hotel=hotel||{};
  ...
  const occ=roomOccupancyText(o);
  const amens=roomAmenitiesList(o);
  const rows=[];
  function push(label,val){
    const t=firstText(val);
    if(!t) return;
    rows.push('<div class="tyh-kv"><span>'+esc(label)+'</span><b>'+esc(t)+'</b></div>');
  }
  push('Room', o.roomSummary||o.roomType||'Selected room');
  push('Meal plan', meal);
  push('Refundability', refundLabel);
  push('Cancellation', cancelLine);
  push('Occupancy', occ||(cap>0?('Fits max. '+cap+' guests'):''));
  push('Bed / room', bedType);
  if(amens.length) rows.push('<div class="tyh-kv tyh-kv-block"><span>Inclusions</span><b>'+esc(amens.slice(0,12).join(' · '))+'</b></div>');
  if(optionHasPan(o)) push('Documents', 'PAN required for this rate');
  const price=hotelBaseFareAmount(o, hotel)||o.displayTotal||o.resultDisplayAmount||o.totalPrice;
  if(Number(price)>0) push('Room Price', fareMoney(price));
  if(!rows.length) return '<p class="tyh-muted">No additional details were returned for this room.</p>';
  return '<div class="tyh-room-details-sheet">'+rows.join('')+(arr(c.penalties).length?cancellationPolicyBody(o):'')+'</div>';
}
function roomRateArticle(o, allowContinue, hotel){
  ...
  const cap=roomCapacityMax(o);
  const viewMore=id
    ? '<button type="button" class="tyh-view-more-link" data-open-room-details="'+attr(id)+'">View details</button>'
    : '';
  const cancelMore=canViewCancelMore&&id
    ? '<button type="button" class="tyh-view-more-link" data-open-cancel-sheet="'+attr(id)+'">View cancellation</button>'
    : '';
...
function roomRatesHtml(h){
  if(!hotelDetailContextMatches(h)){
    return '<section class="tyh-rooms-block" id="tyhRoomTypes"><div class="tyh-rooms-head"><h3>Room types</h3><p class="tyh-muted">Loading live room rates…</p></div></section>';
  }
...
function shell(content, opts){
  ...
  const statusTone=String(opts.statusTone||'').toLowerCase();
  const statusPill=opts.status
    ? ('<span class="tyh-status-pill'+(statusTone?' '+esc(statusTone):'')+'">'+esc(opts.status)+'</span>')
    : '';
  const header = mobileResults
    ? ''
    : '<header class="tyh-top"><button type="button" class="tyh-back" data-back>‹</button>'+(hideLogo?'':logoHtml())+'<div class="tyh-top-title"><h1>'+esc(title)+'</h1>'+(subText?'<p>'+esc(subText)+'</p>':'')+'</div>'+statusPill+'</header>';
...
function bindBase(){
  function goBackOneStep(){
    ...
    if(step==='guest'){
      const fromDraft=hotel();
      const hid=realHotelId(fromDraft||S.detailHotel||S.selectedHotel||{});
      ...
      // Same hotel with ready live rates: restore Detail without wiping draft.
      if(realHotelId(S.detailHotel)===hid && S.detailStatus==='ready' && hotelDetailContextMatches(S.detailHotel) && optionList(S.detailHotel).some(function(o){ return !!realOptionId(o); })){
        renderHotelDetailsPlumbing();
        return;
      }
      // HID mismatch or missing rates: strip foreign options and reload THIS hotel only.
      let next=null;
      if(realHotelId(fromDraft)===hid) next=Object.assign({}, fromDraft);
      else if(realHotelId(S.detailHotel)===hid) next=S.detailHotel;
      else next=hotelByRealId(hid);
      next=stripHotelPricingState(next||{});
      if(!realHotelId(next)){
        setPage('results');
        if(S.shown&&S.shown.length) renderResults(); else loadResults();
        return;
      }
      const seq=beginHotelDetailSwitch(hid);
      S.detailHotel=next;
      S.detailStatus='loading';
      S.detailError='';
      renderHotelDetailsPlumbing();
      loadHotelPricing(next, seq);
      return;
    }
...
function maybeShowHotelDetailsFromUrl(){
  ...
  if(hid && String(S.detailRequestHid||'')!==hid) beginHotelDetailSwitch(hid);
  const found=stripHotelPricingState(hotelByRealId(hid)||{});
  if(!realHotelId(found)) return false;
  S.detailHotel=found;
  S.detailStatus='loading';
  S.detailError='';
  renderHotelDetailsPlumbing();
  loadHotelPricing(found, S.detailRequestSeq);
  return true;
}
...
function hotelDescription(h){
  const r=rawOf(h);
  const content=r.content||r.hotelContent||h.content||{};
  const descObj=content.descriptions||h.descriptions||r.descriptions||{};
  return firstText(
    h.description, content.description, descObj.overview, descObj.description,
    h.des, r.des, r.description, deepFind(r,['description','des','about','overview'])
  );
}
function hotelAboutSectionsList(h){
  h=h||{};
  const raw=rawOf(h);
  const content=raw.content||raw.hotelContent||h.content||{};
  const descObj=content.descriptions||h.descriptions||raw.descriptions||{};
  const pol=policiesOf(h)||{};
  const sections=[];
  function push(title, val){
    let text='';
    if(Array.isArray(val)){
      text=val.map(function(x){ return customerSafeNote(firstText(x&&x.msg,x&&x.description,x&&x.text,x&&x.name,x&&x.label,x)); }).filter(Boolean).join('\n');
    } else if(val && typeof val==='object'){
      text=customerSafeNote(firstText(val.text,val.description,val.body,val.msg,val.name));
    } else {
      text=customerSafeNote(firstText(val));
    }
    if(!text) return;
    if(/tripjack|supplier|backend|\bapi\b/i.test(text)) return;
    if(sections.some(function(s){ return s.text===text; })) return;
  ...
  if(descObj && typeof descObj==='object' && !Array.isArray(descObj)){
    push('About the property', descObj.overview||descObj.description);
    push('About the property', descObj.headline);
    push('Location', descObj.location);
    push('Nearby attractions', descObj.attractions||descObj.nearby_attractions||descObj.nearbyAttractions);
    push('Amenities', descObj.amenities);
    push('Rooms', descObj.rooms);
    push('Dining', descObj.dining);
    push('Business amenities', descObj.business_amenities||descObj.businessAmenities);
    push('Onsite payments', descObj.onsite_payments||descObj.onsitePayments);
    push('Spoken languages', descObj.spoken_languages||descObj.spokenLanguages);
    push('Check-in information', descObj.check_in||descObj.checkIn||descObj.check_in_information);
  }
  ...
  push('Nearby attractions', content.attractions||content.nearbyAttractions||content.areaAttractions);
  ...
  push('Check-in information', pol.checkInInstructions||pol.checkinInstructions||content.checkInInstructions||content.checkInInformation);
  push('Check-out information', pol.checkOutInstructions||pol.checkoutInstructions||content.checkOutInstructions);
  arr(content.sections).concat(arr(h.descriptions), arr(raw.descriptions)).forEach(function(sec){
    if(!sec||typeof sec!=='object' || Array.isArray(sec)) return;
    if('headline' in sec || 'location' in sec || 'attractions' in sec) return;
...
function unmountGuestFareSheetPortal(){
  const el=document.getElementById('tyhFareSheetPortal');
  if(el&&el.parentNode) el.parentNode.removeChild(el);
}
function guestFareSheetPortalHtml(parts){
  parts=parts||hotelFareParts();
...
function mountGuestFareSheetPortal(){
  ...
  wrap.id='tyhFareSheetPortal';
  wrap.innerHTML=guestFareSheetPortalHtml(hotelFareParts());
  document.body.appendChild(wrap);
}
function bindGuestFareSheetPortal(){
  const portal=document.getElementById('tyhFareSheetPortal');
  if(!portal) return;
  qa('[data-fare-sheet-close]',portal).forEach(function(b){
    b.onclick=function(e){ e.preventDefault(); e.stopPropagation(); closeGuestFareSheet(); };
  });
}
function openGuestFareSheet(){
  S.ui.guestScrollY=window.scrollY||0;
  S.ui.fareSheetOpen=true;
  mountGuestFareSheetPortal();
  document.body.classList.add('tyh-modal-lock');
  bindGuestFareSheetPortal();
  const chev=q('.tyh-fare-chevron',root);
  if(chev) chev.classList.add('is-open');
}
function closeGuestFareSheet(){
  S.ui.fareSheetOpen=false;
  unmountGuestFareSheetPortal();
  document.body.classList.remove('tyh-modal-lock');
  const chev=q('.tyh-fare-chevron',root);
  if(chev) chev.classList.remove('is-open');
  const y=Number(S.ui.guestScrollY||0);
...
function renderHotelDetailsPlumbing(){
  ...
  const aboutSections=hotelAboutSectionsList(h);
  const aboutPreview=hotelAboutPreviewHtml(h);
  const aboutHasMore=aboutSections.length>1 || ((aboutSections[0]||{}).text||'').length>240;
  ...
  const aboutBlock=aboutPreview
    ? '<section class="tyh-panel tyh-about"><h2>About this property</h2><div class="tyh-about-body" data-about-body>'+aboutPreview+'</div>'
      +(aboutHasMore?'<button type="button" class="tyh-linkish" data-open-hotel-sheet="about">Show more</button>':'')
    +'</section>'
    : '';
  ...
  if(rec&&hasPricingReviewContext(h)){
    ...
    bookBox='<aside class="tyh-book-box">'
      +'<div class="tyh-book-box-room"><b>'+esc(rec.roomSummary||rec.roomType||'Recommended room')+'</b>'
        +(rid?'<button type="button" class="tyh-view-more-link" data-open-room-details="'+attr(rid)+'">View details</button>':'')
        +(occ?'<p>'+esc(occ)+'</p>':'')
...
function bindHotelDetailExtras(){
  ...
  qa('[data-open-hotel-sheet]',root).forEach(function(b){
    b.onclick=function(e){
      ...
      if(type==='about'){
        const full=hotelAboutSectionsHtml(hh);
        openHotelSheet('About this property', full||'<p class="tyh-muted">No description available.</p>');
        return;
      }
      if(type==='description'){
  ...
  qa('[data-open-cancel-sheet]',root).forEach(function(b){
    b.onclick=function(e){
      ...
      const hh=S.detailHotel||{};
      if(!hotelDetailContextMatches(hh)) return;
  ...
  qa('[data-open-room-details]',root).forEach(function(b){
    b.onclick=function(e){
      e.preventDefault();
      e.stopPropagation();
      const optId=String(b.getAttribute('data-open-room-details')||'');
      const hh=S.detailHotel||{};
      if(!hotelDetailContextMatches(hh)) return;
      const opt=optionList(hh).find(function(o){ return realOptionId(o)===optId; });
      if(!opt) return;
      openHotelSheet(opt.roomSummary||opt.roomType||'Room details', roomOptionDetailsHtml(opt, hh));
    };
  });
  ...
  if(aboutBtn) aboutBtn.onclick=function(){
    const full=hotelAboutSectionsHtml(S.detailHotel||{});
    openHotelSheet('About this property', full||'<p class="tyh-muted">No description available.</p>');
  };
...
function openHotelDetails(h){
  ...
  if(!id) return;
  const reqSeq=beginHotelDetailSwitch(id);
  h=stripHotelPricingState(h);
  S.detailHotel=h;
  S.detailStatus='loading';
  S.detailError='';
  ...
  renderHotelDetailsPlumbing();
  loadHotelPricing(h, reqSeq);
}
let hotelDetailRequestSeq=0;
function currentUrlHotelId(){
  return String(new URLSearchParams(location.search).get('hotelId')||'').trim();
}
function hotelDetailContextMatches(h){
  const hid=realHotelId(h);
  const urlHid=currentUrlHotelId();
  if(currentStep()!=='hotel-details') return false;
  if(!urlHid || !hid) return false;
  return hid===urlHid;
}
function stripHotelPricingState(hotel){
  const h=Object.assign({}, hotel||{});
  const hid=realHotelId(h);
  delete h.options;
  h.reviewHash='';
  delete h.pricingBreakup;
  delete h.priceBreakup;
  const raw=Object.assign({}, h.raw||{});
  delete raw.options;
  delete raw.option;
  delete raw.ops;
  if(raw.hInfo) raw.hInfo=Object.assign({}, raw.hInfo, {ops:[]});
  if(raw.data && raw.data.hInfo) raw.data=Object.assign({}, raw.data, {hInfo:Object.assign({}, raw.data.hInfo, {ops:[]})});
  h.raw=raw;
  if(hid){ h.hotelId=hid; h.id=hid; h.tjHotelId=hid; }
  return h;
}
function clearStaleHotelBookingState(nextHid){
  // Drop Hotel A draft locks/options so Hotel B Detail never paints A's prices/options.
  S.selectedHotel=null;
  S.selectedOption=null;
  S.review=null;
  S.roomHotel=null;
  ...
  try{
    const d=draft()||{};
    const prevHid=realHotelId(d.hotel||d.selected||{});
    if(prevHid && nextHid && prevHid===String(nextHid)) return;
...
function beginHotelDetailSwitch(hid){
  ...
  hotelDetailRequestSeq+=1;
  S.detailRequestSeq=hotelDetailRequestSeq;
  S.detailRequestHid=nextHid;
  S.ui.fareSheetOpen=false;
  unmountGuestFareSheetPortal();
  clearStaleHotelBookingState(nextHid);
  // Drop previous hotel's painted detail immediately so Hotel B never briefly shows Hotel A rooms/prices.
  if(realHotelId(S.detailHotel) && realHotelId(S.detailHotel)!==nextHid){
    S.detailHotel=null;
    S.detailStatus='loading';
    S.detailError='';
    S.roomHotel=null;
  }
  return hotelDetailRequestSeq;
}
function isActiveHotelDetailRequest(hid, seq){
  return Number(seq)===Number(S.detailRequestSeq||0) && String(hid||'')===String(S.detailRequestHid||'');
}
function tyhStillOnHotelDetails(hotelId){
  if(currentStep()!=='hotel-details') return false;
  if(hotelId==null || hotelId==='') return false;
  return currentUrlHotelId()===String(hotelId);
}
function applyPricedHotel(listingHotel, merged, reqSeq){
  const id=realHotelId(listingHotel);
  if(!merged || realHotelId(merged)!==id) return;
  if(!isActiveHotelDetailRequest(id, reqSeq==null?S.detailRequestSeq:reqSeq)) return;
  if(!tyhStillOnHotelDetails(id)) return;
  merged=mergeHotelKeepMedia(listingHotel, merged);
  if(realHotelId(merged)!==id || !hotelDetailContextMatches(merged)) return;
...
function mergeHotelKeepMedia(prev, next){
  prev=prev||{}; next=next||{};
  const prevId=realHotelId(prev);
  const nextId=realHotelId(next);
  if(prevId && nextId && prevId!==nextId) prev=stripHotelPricingState(prev);
  const merged=Object.assign({}, prev, next);
  merged.options=arr(next.options).length?next.options:(prevId===nextId?prev.options:[]);
  merged.reviewHash=String(next.reviewHash||'').trim()||(prevId===nextId?prev.reviewHash:'');
  ...
  if(prevId && nextId && prevId!==nextId){
    merged.content=next.content||prev.content;
    merged.description=next.description||prev.description;
  }
  ...
  const nextRaw=merged.raw||next.raw||{};
  const prevRaw=prev.raw||{};
  const nextOps=arr(next.options).length||arr(nextRaw.ops).length||arr(nextRaw.hInfo&&nextRaw.hInfo.ops).length;
  if(prev.raw && (!merged.raw || !allHotelImages({raw:merged.raw}).length) && prevImgs.length){
    ...
    merged.raw=Object.assign({}, prevRaw, nextRaw, {
      content:Object.assign({}, prevRaw.content||{}, nextRaw.content||{}),
      policies:nextRaw.policies||prevRaw.policies,
      hInfo:Object.assign({}, prevRaw.hInfo||{}, nextRaw.hInfo||{})
    });
  }
  if(merged.raw){
    if(nextOps){
      merged.raw=Object.assign({}, merged.raw);
      merged.raw.ops=arr(nextRaw.ops).length?nextRaw.ops:arr(nextRaw.hInfo&&nextRaw.hInfo.ops);
      if(merged.raw.hInfo) merged.raw.hInfo=Object.assign({}, merged.raw.hInfo, {ops:merged.raw.ops});
    } else if(prevId && nextId && prevId!==nextId){
      merged.raw=Object.assign({}, merged.raw, {ops:[], option:null, options:[]});
      if(merged.raw.hInfo) merged.raw.hInfo=Object.assign({}, merged.raw.hInfo, {ops:[]});
    }
  }
  return merged;
}
async function loadHotelPricing(h, reqSeq){
  const id=realHotelId(h);
  const seq=reqSeq==null?S.detailRequestSeq:reqSeq;
  S.detailStatus='loading';
  S.detailError='';
  if(isActiveHotelDetailRequest(id, seq) && tyhStillOnHotelDetails(id)) renderHotelDetailsPlumbing();
  const merged=await openRooms(h, true, seq);
  if(!isActiveHotelDetailRequest(id, seq) || !tyhStillOnHotelDetails(id)) return;
  if(S.detailStatus==='error') return;
  applyPricedHotel(h, merged, seq);
}
...
async function openRooms(h, silent, reqSeq){
  ...
  if(hotelDetailLock && hotelDetailLock.key===lockKey && hotelDetailLock.seq===seq) return hotelDetailLock.promise;
  const job={key:lockKey, seq:seq, promise:null};
  job.promise=(async function(){
  try{
    ...
    if(silent && (!isActiveHotelDetailRequest(hid, seq) || !tyhStillOnHotelDetails(hid))) return null;
    const d=unwrap(res)||{};
    const priced=d.hotel||{};
    const responseHid=String(priced.hotelId||priced.tjHotelId||d.hotelId||d.hid||hid||'').trim();
    if(responseHid && String(responseHid)!==String(hid)) return null;
    ...
    if(realHotelId(merged)!==hid) return null;
    if(!isActiveHotelDetailRequest(hid, seq) || !tyhStillOnHotelDetails(hid)) return null;
    ...
    if(silent && (!isActiveHotelDetailRequest(id, seq) || !tyhStillOnHotelDetails(id))) return null;
    hideLoader();
...
function renderGuestStep(){
  ...
  const panBlock=showPan?('<section class="tyh-panel tyh-pan-panel">'
    ...
    +'</div>'
    +'<div class="tyh-form-grid"><label><span class="tyh-field-label"><span class="tyh-lab-text">Name (Room '+(g.room||1)+')</span>'+(needPan?'<span class="tyh-req" aria-hidden="true">*</span>':'')+'</span><input data-gfield="panName" value="'+attr(g.panName||[g.firstName,g.lastName].filter(Boolean).join(' ')) +'" placeholder="Name"'+(needPan?' required':'')+'></label>'
    +'<label><span class="tyh-field-label"><span class="tyh-lab-text">PAN number</span>'+(needPan?'<span class="tyh-req" aria-hidden="true">*</span>':'')+'</span><input data-gfield="pan" value="'+attr(g.pan||'')+'" placeholder="PAN Number" autocomplete="off"'+(needPan?' required':'')+'></label></div>'
  ...
  const leftMain='<div class="tyh-guest-main">'
    ...
    +'<section class="tyh-panel tyh-guest-panel">'
      ...
      +'<div class="tyh-form-grid tyh-guest-names-grid" data-guest-form="'+i+'">'
        +'<label class="tyh-guest-title"><span class="tyh-field-label"><span class="tyh-lab-text">Title</span><span class="tyh-req" aria-hidden="true">*</span></span><select data-gfield="title" required><option '+(g.title==='Mr'?'selected':'')+'>Mr</option><option '+(g.title==='Ms'?'selected':'')+'>Ms</option><option '+(g.title==='Mrs'?'selected':'')+'>Mrs</option><option '+(g.title==='Master'?'selected':'')+'>Master</option></select></label>'
        +'<label><span class="tyh-field-label"><span class="tyh-lab-text">First Name</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-gfield="firstName" value="'+attr(g.firstName||'')+'" placeholder="First Name" autocomplete="given-name" required></label>'
        +'<label><span class="tyh-field-label"><span class="tyh-lab-text">Last Name</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-gfield="lastName" value="'+attr(g.lastName||'')+'" placeholder="Last Name" autocomplete="family-name" required></label>'
        +(g.type==='Child'?'<label><span class="tyh-field-label"><span class="tyh-lab-text">Age</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-gfield="age" value="'+attr(g.age||'')+'" placeholder="Child age" inputmode="numeric" required></label>':'')
        +(needPass?'<label><span class="tyh-field-label"><span class="tyh-lab-text">Passport Number</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-gfield="passport" value="'+attr(g.passport||'')+'" placeholder="Passport Number" autocomplete="off" required></label>':'')
      +'</div>'
    ...
    +'<section class="tyh-panel tyh-contact-panel">'
      ...
      +'<div class="tyh-form-grid tyh-contact-grid">'
        +'<label class="tyh-contact-email"><span class="tyh-field-label"><span class="tyh-lab-text">Email</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-contact="email" type="email" value="'+attr(contact.email||'')+'" placeholder="Email" autocomplete="email" required></label>'
        +'<div class="tyh-phone-row">'
          +'<label class="tyh-ccode"><span class="tyh-field-label"><span class="tyh-lab-text">Country Code</span><span class="tyh-req" aria-hidden="true">*</span></span><select data-contact="countryCode" required>'+countryCodeOptionsHtml(contact.countryCode||'+91')+'</select></label>'
          +'<label class="tyh-contact-phone"><span class="tyh-field-label"><span class="tyh-lab-text">Mobile Number</span><span class="tyh-req" aria-hidden="true">*</span></span><input data-contact="phone" inputmode="numeric" value="'+attr(contact.phone||'')+'" placeholder="Mobile number" autocomplete="tel" required></label>'
        +'</div>'
      +'</div>'
  ...
  const content='<main class="tyh-book tyh-book-flow'+(desktop?' tyh-guest-desk':'')+'">'
    ...
    +(!desktop
      ? '<div class="tyh-bottom"><button type="button" class="tyh-fare-tap" data-open-fare-sheet><span class="tyh-fare-tap-label">Payment Details <i class="tyh-fare-chevron'+(S.ui.fareSheetOpen?' is-open':'')+'" aria-hidden="true">▴</i></span><b>'+fareMoney(finalPay)+'</b></button><button type="button" class="'+(complete?'':'tyh-cta-soft-disabled')+'" data-pay'+(complete?' aria-disabled="false"':' aria-disabled="true"')+'>Continue to payment</button></div>'
      : '')
  +'</main>';

  shell(content,{title:'Review Your Booking', hideLogo:true, sub:''});
  bindGuest();
  if(!desktop && S.ui.fareSheetOpen){
    const portal=document.getElementById('tyhFareSheetPortal');
    if(portal){
      portal.innerHTML=guestFareSheetPortalHtml(hotelFareParts());
      bindGuestFareSheetPortal();
      document.body.classList.add('tyh-modal-lock');
    }
  } else if(!S.ui.fareSheetOpen){
    unmountGuestFareSheetPortal();
  }
  ...
  if(applied){
    ...
    return '<section class="tyh-offers-card">'
      ...
      +'<article class="tyh-offer-item applied">'
        +'<div><b>'+esc(code)+'</b><span>Applied</span><p>You saved '+fareMoney(applied.discountAmount||0)+'</p></div>'
...
function hotelMobileOfferLineHtml(applied){
  if(applied){
    const code=applied.offerCode||applied.code||'';
    return '<section class="tyh-panel tyh-offer-mobile"><button type="button" class="tyh-offer-mobile-link" data-open-mobile-offer>TravelYaraa Offer · '+esc(code)+' applied ›</button></section>';
  }
  const offers=eligibleHotelFirstOffers();
  if(!offers.length) return '';
  const label=firstText(offers[0]&&offers[0].title, offers[0]&&offers[0].code, offers[0]&&offers[0].offerCode)||'Apply offer';
  return '<section class="tyh-panel tyh-offer-mobile"><button type="button" class="tyh-offer-mobile-link" data-open-mobile-offer>TravelYaraa Offer · '+esc(label)+' ›</button></section>';
}
function eligibleHotelFirstOffers(){
  return arr(S.ui.hotelOffers).filter(function(o){
    if(!o) return false;
    if(S.ui.hotelOfferIneligible) return false;
    if(o.eligible===false || o.isEligible===false || o.available===false) return false;
    if(o.consumed===true || o.alreadyUsed===true || o.used===true) return false;
...
async function refreshHotelOffers(){
  try{
    const res=await fetch(API+'/api/offers?service=hotel',{cache:'no-store',headers:requestHeaders('/api/offers',false)});
    const data=await res.json().catch(function(){ return {}; });
    ...
    S.ui.hotelOfferIneligible=!offers.some(function(o){
      const first=o.firstBookingOnly===true || o.firstBookingOnly===1 || String(o.firstBookingOnly||'').toLowerCase()==='true';
      const neu=o.newUserOnly===true || o.newUserOnly===1 || String(o.newUserOnly||'').toLowerCase()==='true';
      return first || neu;
    });
...
async function applyHotelOffer(code){
  ...
  const feeAfter=res.convenienceFeeWaived?0:(Number(res.convenienceFee||res.travelYaraaServiceFee||0)>0?Number(res.convenienceFee||res.travelYaraaServiceFee):serviceFee);
  ...
  if(!res || (discount<=0 && !res.finalPayableAmount && !res.convenienceFeeWaived)) throw new Error(res&&res.message||'Offer could not be applied.');
  setDraft({
...
function bindMobileOfferSheetActions(){
  const portal=document.getElementById('tyhSheetPortal');
  if(!portal) return;
  const applySel=q('[data-apply-hotel-offer-selected]',portal);
  if(applySel) applySel.onclick=async function(){
    const sel=q('[data-hotel-offer-select]',portal);
    const code=sel&&sel.value;
    if(!code) return;
    try{ showLoader('Validating offer...'); await applyHotelOffer(code); closeHotelSheet(); }
    catch(e){ closeHotelSheet(); await handleHotelOfferApplyError(e); }
    finally{ hideLoader(); }
  };
  const rem=q('[data-remove-hotel-offer]',portal);
  if(rem) rem.onclick=function(){ removeHotelOffer(); closeHotelSheet(); };
}
...
function hotelOfferErrorMessage(err){
  ...
  if(!msg) return 'This offer cannot be applied to the current booking.';
  if(/tripjack|supplier|backend|\bapi\b|razorpay/i.test(msg)) return 'This offer cannot be applied to the current booking.';
  return msg;
}
async function handleHotelOfferApplyError(err){
  hideLoader();
  if(String(err&&err.code||'')==='FIRST_BOOKING_ONLY' || /first successful booking/i.test(String(err&&err.message||''))){
    S.ui.hotelOfferIneligible=true;
    S.ui.hotelOffers=[];
    if(currentStep()==='guest'||currentStep()==='review') renderGuestStep();
  }
  if(hotelOfferErrorIsLogin(err)){
    await showHotelNotifyModal({
      type:'notice',
      title:'Sign in to use this offer',
      message:'Please sign in to check your eligibility and apply this offer.',
      primary:'OK',
      secondary:'Close'
    });
    return;
  ...
  await showHotelNotifyModal({
    ...
    message:hotelOfferErrorMessage(err),
    primary:'OK',
    secondary:'Close'
  });
}
function bindGuest(){
  ...
  qa('[data-apply-hotel-offer]',root).forEach(function(b){
    b.onclick=async function(){
      ...
      catch(e){ await handleHotelOfferApplyError(e); }
      finally{ hideLoader(); }
    };
  });
  ...
  qa('[data-open-fare-sheet]',root).forEach(function(b){ b.onclick=function(e){ e.preventDefault(); openGuestFareSheet(); }; });
  qa('[data-open-mobile-offer]',root).forEach(function(b){
    b.onclick=function(e){
      e.preventDefault();
      const appliedNow=draft().appliedOffer||null;
      openHotelSheet('TravelYaraa Offer', hotelCouponHtml(appliedNow)||'<p class="tyh-muted">No offers available.</p>');
      bindMobileOfferSheetActions();
    };
  });
  qa('[data-pay]',root).forEach(function(cont){
    syncPayState();
...
function hotelMiniCard(h,o){
  ...
  const cell=function(label,value,timeVal,timeLabel){
    return '<div class="tyh-stay-cell"><small>'+esc(label)+'</small><b>'+esc(value)+'</b>'
      +(timeVal?'<span class="tyh-stay-time"><small class="tyh-stay-time-lab">'+esc(timeLabel||'Time')+'</small><strong>'+esc(timeVal)+'</strong></span>':'')
    +'</div>';
  };
  ...
  const stayHtml=desk
    ? ('<div class="tyh-stay-strip tyh-stay-desk" aria-label="Stay summary">'
        +'<div class="tyh-stay-mainrow">'
          +cell('Check In', fmtDate(ci)||'Pending', times.checkIn, 'Check-in time')
          +nightPill
          +cell('Check Out', fmtDate(co)||'Pending', times.checkOut, 'Check-out time')
...
function hotelPaymentPaid(b){
  const p=String((b&&b.paymentStatus)||(b&&b.payment&&(b.payment.status||b.payment.paymentStatus))||'').toUpperCase();
...
function hotelPendingStale(b,ms){
  const t=Date.parse((b&&(b.updatedAt||b.createdAt))||'')||0;
  if(!t) return true;
  return (Date.now()-t)>(ms||30*60*1000);
}
function hotelDetailKind(b){
  ...
  // Paid and waiting for hotel confirmation — only briefly.
  if(hotelPendingStale(b)) return 'failed_paid';
  return 'pending_paid';
}
...
function hotelCanShowReceipt(b){
  const k=hotelDetailKind(b);
  if(!hotelPaymentPaid(b)) return false;
  if(k==='pending'||k==='pending_paid'||k==='failed_unpaid') return false;
  return true;
}
...
function hotelRefundSectionHtml(b){
  ...
  const timeline=firstVal(b.refundTimeline,b.refundEta,b.payment&&b.payment.refundTimeline,b.payment&&b.payment.refundEta,'');
  const cancelledAt=firstVal(b.cancelledAt,b.cancellationRequest&&b.cancellationRequest.requestedAt,'');
  const hasData=!!(status||amount>0||charge>0||when||method||timeline);
  const emptyMsg=kind==='failed_paid'
    ? 'Refund details are not available yet. Please contact support with your Booking ID.'
    : 'Your booking has been cancelled. Refund details are not available yet. Please contact support with your Booking ID.';
  const hasMsg=kind==='failed_paid'
    ? 'Payment was received, but the hotel booking could not be confirmed. Refund details are shown below.'
    : 'Your booking has been cancelled. Refund details are shown below.';
  let html='<section class="tyh-bd-card" id="tyhRefundBox"><h3>Refund info</h3>';
  if(!hasData){
    html+='<p class="tyh-bd-empty">'+esc(emptyMsg)+'</p></section>';
    return html;
  }
  html+='<p class="tyh-bd-note">'+esc(hasMsg)+'</p>';
  ...
  if(timeline) html+='<div class="tyh-kv"><span>Expected refund timeline</span><b>'+esc(timeline)+'</b></div>';
...
function renderStatus(b, raw){
  ...
  if(kind==='upcoming'){ statusTitle='Booking Confirmed'; statusNote='Your hotel booking is confirmed.'; }
    ...
    if(nonRefundable) statusNote='This booking is non-refundable as per the hotel cancellation policy.';
  ...
  const placeLine=firstVal(addressFromHotel(h), (b.selectedResult&&(b.selectedResult.city||b.selectedResult.address||b.selectedResult.area)), hotelStatusQuery(b).cityName, hotelStatusQuery(b).city, (b.details&&b.details.searchPayload&&(b.details.searchPayload.cityName||b.details.searchPayload.city)), '');
  const placeHtml=placeLine
    ? '<p class="tyh-bd-address">'+esc(placeLine)+'</p>'
    : '<p class="tyh-bd-address">Address will appear when available for this booking.</p>';

  const actionRows=[];
  if(canVoucher) actionRows.push(['▤','Download voucher','download','/api/bookings/'+id+'/ticket','']);
  if(canReceipt) actionRows.push(['▤','Download invoice','download','/api/bookings/'+id+'/invoice','']);
  if(canConfirm) actionRows.push(['✉','Resend email','action','confirm-email','']);
  if(canCancel) actionRows.push(['⊘','Cancel booking','action','cancel','danger']);
  if(kind==='pending'||kind==='pending_paid'||kind==='failed_paid') actionRows.push(['↻','Check status','action','refresh','']);
  if(hotelNeedsRefundSection(b) && paid && !nonRefundable) actionRows.push(['₹','View refund status','action','refund-status','']);
  if(kind==='cancelled'&&paid) actionRows.push(['▤','Download cancellation receipt','download','/api/bookings/'+id+'/receipt','']);
  actionRows.push(['←','Back to My Bookings','action','my-bookings','']);
  actionRows.push(['☎','Contact support','action','support','']);

  const actionsHtml=actionRows.map(function(row){
    const ico=row[0], label=row[1], kindA=row[2], value=row[3], tone=row[4];
    const cls='tyh-bd-manage-row'+(tone==='danger'?' is-danger':'')+(tone==='primary'?' is-primary':'');
    if(kindA==='download') return '<button type="button" class="'+cls+'" data-download="'+attr(value)+'"><span class="ico" aria-hidden="true">'+ico+'</span><span>'+esc(label)+'</span></button>';
    return '<button type="button" class="'+cls+'" data-action="'+attr(value)+'"><span class="ico" aria-hidden="true">'+ico+'</span><span>'+esc(label)+'</span></button>';
  }).join('');
  ...
  const statusSection =
    '<section class="tyh-bd-card tyh-bd-status '+esc(lab[1])+'" id="tyhSecStatus">'
      ...
      +'<div class="tyh-bd-status-body">'
        ...
        +(nonRefundable?'<div class="tyh-bd-policy-alert bad"><b>Non-refundable</b><p>This booking is non-refundable as per the hotel cancellation policy.</p></div>':'')
        +bookAgainCta
      +'</div>'
    +'</section>';
  ...
  const cancelSection =
    '<section class="tyh-bd-section" id="tyhSecCancel" data-bd-section="cancel">'
      ...
      +(nonRefundable?'<div class="tyh-bd-policy-alert bad"><b>Non-refundable</b><p>This booking is non-refundable as per the hotel cancellation policy.</p></div>':'')
      +(kind==='cancelled'?'<div class="tyh-bd-policy-alert bad"><b>This booking is cancelled</b><p>'+esc(statusNote)+'</p></div>':'')
  ...
  const actionsSection =
    '<section class="tyh-bd-section" id="tyhSecActions">'
      +'<section class="tyh-bd-card tyh-bd-actions-card"><h3>Actions</h3>'
      +'<div class="tyh-bd-manage-list">'+actionsHtml+'</div>'
      +'</section></section>';
  ...
  shell(content,{title:'Booking Detail',sub:'Hotel booking',status:statusTitle,statusTone:lab[1],hideLogo:true});
  bindStatus(id,b);
...
function init(){
  ...
  if(step==='hotel-details'){
    ...
    const found=stripHotelPricingState(hotelByRealId(hid)||{});
    S.detailHotel=realHotelId(found)?found:null;
    S.detailStatus=realHotelId(found)?'loading':'idle';
    S.detailError='';
    renderHotelDetailsPlumbing();
    if(realHotelId(found)) loadHotelPricing(found, S.detailRequestSeq);
    loadResults();
    return;
  }
  setPage('results');
...
.tyh-form-grid label{display:flex;flex-direction:column;align-items:stretch;gap:6px;color:inherit}
.tyh-field-label{display:inline-flex;align-items:baseline;gap:3px;font-size:13px;font-weight:800;line-height:1.3}
.tyh-lab-text{color:#344054}
.tyh-req{display:inline!important;margin:0;padding:0;border:0;background:transparent;color:#b42318!important;font-weight:900!important;font-size:13px;line-height:1;-webkit-text-fill-color:#b42318!important}
.tyh-stay-time{display:block;margin-top:6px;color:var(--ty-navy);font-size:12px;font-weight:800;font-style:normal;line-height:1.35}
.tyh-stay-time-lab{display:block;margin:0 0 2px;color:var(--ty-muted);font-size:10px;font-weight:800;letter-spacing:.03em;text-transform:uppercase}
.tyh-stay-time strong{display:block;color:var(--ty-navy);font-size:13px;font-weight:900}
.tyh-offer-hint{margin:0 0 8px;color:#98a2b3;font-size:11px;font-weight:650;line-height:1.35}
...
.tyh-view-more-link,.tyh-hotel-rules-link,.tyh-view-rooms-btn,.tyh-linkish{display:inline!important;border:0!important;background:none!important;padding:0!important;margin:8px 0 0!important;color:var(--ty-blue)!important;font-size:13px!important;font-weight:800!important;cursor:pointer;text-decoration:underline!important;text-underline-offset:3px;width:auto!important;height:auto!important;min-height:0!important;border-radius:0!important;box-shadow:none!important;-webkit-appearance:none;appearance:none;outline:0}
...
.tyh-bd-status-cta{margin-top:14px;display:grid;gap:8px}
.tyh-bd-status-cta.twin{grid-template-columns:1fr 1fr}
.tyh-bd-status-cta .tyh-bd-btn{width:100%;min-height:46px;font-size:15px}
.tyh-bd-manage-list{margin-top:4px;border:1px solid var(--ty-line);border-radius:14px;overflow:hidden;background:#fff}
.tyh-bd-manage-row{display:flex;align-items:center;gap:12px;width:100%;border:0;border-top:1px solid #eef0f4;background:#fff;padding:14px 16px;cursor:pointer;text-align:left;font-size:14px;font-weight:800;color:#111827}
.tyh-bd-manage-row:first-child{border-top:0}
.tyh-bd-manage-row .ico{width:22px;text-align:center;color:#111827;font-size:16px;flex:0 0 auto}
.tyh-bd-manage-row.is-primary{color:var(--ty-blue)}
.tyh-bd-manage-row.is-primary .ico{color:var(--ty-blue)}
.tyh-bd-manage-row.is-danger{color:#b42318}
.tyh-bd-manage-row.is-danger .ico{color:#b42318}
.tyh-bd-actions-card .tyh-bd-status-cta{margin:0 0 12px}
...
.tyh-fare-sheet{position:fixed;left:0;right:0;bottom:0;z-index:241;background:#fff;border-radius:22px 22px 0 0;box-shadow:0 -14px 40px rgba(0,0,0,.2);max-height:78vh;overflow:hidden;display:flex;flex-direction:column}
.tyh-fare-sheet .tyh-sheet-body{overflow:auto;flex:1;min-height:0;-webkit-overflow-scrolling:touch}
#tyhFareSheetPortal{position:fixed;inset:0;z-index:240;pointer-events:none}
#tyhFareSheetPortal .tyh-modal-bg,#tyhFareSheetPortal .tyh-fare-sheet{pointer-events:auto}
...
.tyh-fare-tap-label{display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:800;color:#cbd5e1}
.tyh-fare-chevron{font-style:normal;font-size:10px;line-height:1;color:#cbd5e1;transition:transform .15s ease}
.tyh-fare-chevron.is-open{transform:rotate(180deg)}
.tyh-fare-tap span{font-size:12px;font-weight:800;color:#cbd5e1}
.tyh-fare-tap b{font-size:22px;color:#fff}
.tyh-offer-mobile{padding:10px 12px!important}
.tyh-offer-mobile-link{display:block;width:100%;border:0;background:transparent;padding:0;color:var(--ty-blue)!important;font-weight:800;font-size:13px;text-align:left;cursor:pointer;-webkit-appearance:none;appearance:none}
...
.tyh-policy-actions button,.tyh-hotel-rules-btn,.tyh-hotel-rules-link{border:0!important;background:transparent!important;border-radius:0!important;padding:0!important;margin:0;font-weight:800;font-size:13px;color:var(--ty-blue)!important;cursor:pointer;text-decoration:underline;text-underline-offset:3px;line-height:1.4;-webkit-appearance:none;appearance:none;box-shadow:none!important}
.tyh-policy-actions button:hover,.tyh-hotel-rules-btn:hover,.tyh-hotel-rules-link:hover{color:var(--ty-navy)!important;background:transparent!important}
...
/* Canonical required-star + secondary text-link styles (last wins; do not re-append duplicates below). */
.tyh-form-grid label{color:#344054}
.tyh-field-label{display:inline-flex;align-items:baseline;gap:3px;font-size:13px;font-weight:800;line-height:1.3}
.tyh-lab-text{color:#344054;-webkit-text-fill-color:#344054}
.tyh-req,.tyh-field-label > .tyh-req{display:inline!important;margin:0;padding:0;border:0;background:transparent;color:#b42318!important;-webkit-text-fill-color:#b42318!important;font-weight:900!important;font-size:13px;line-height:1}
.tyh-view-more-link,.tyh-hotel-rules-link,.tyh-view-rooms-btn,.tyh-linkish,.tyh-add-guest,.tyh-offer-mobile-link,.tyh-policy-actions button{
  display:inline!important;border:0!important;background:none!important;padding:0!important;color:var(--ty-blue)!important;font-weight:800!important;cursor:pointer;text-decoration:underline!important;text-underline-offset:3px;border-radius:0!important;box-shadow:none!important;-webkit-appearance:none;appearance:none;outline:0;min-height:0!important;height:auto!important;width:auto!important
}
`; }
